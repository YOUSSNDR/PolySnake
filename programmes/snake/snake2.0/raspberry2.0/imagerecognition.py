...
def arrow_check():
    ...
    for i in range(len(contours)):
        ...
        if len(approx) == 7:
            ...
            cx, cy = inrectangle(contours[i],pic)
            cx, cy = int(cx), int(cy)
...
#cv2.imshow("pic", pic)
#cv2.waitKey(0)#on le mettra a 1 pour que l'image se ferme toute seule après traitement
...
"""while True:
    ...
    if distance < limit:
        ...
        decision=arrow_check()  ## start the image recognition protocol
